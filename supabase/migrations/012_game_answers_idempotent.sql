-- =============================================================================
-- IDEMPOTENCIA EN game_answers - Evitar error "duplicate key" al responder
-- =============================================================================
-- Un estudiante solo debe poder enviar UNA respuesta por pregunta por sesión.
-- Si hay doble clic, reconexión o reintento, tratamos como éxito (no fallar).
-- =============================================================================

-- 0. Asegurar que level_id existe (la función lo usa)
ALTER TABLE public.game_answers ADD COLUMN IF NOT EXISTS level_id integer DEFAULT 1;

-- 1. Eliminar duplicados existentes (conservar el primero por session+question)
...
-- 2. Crear índice único si no existe (uno por sesión+pregunta)
CREATE UNIQUE INDEX IF NOT EXISTS game_answers_session_question_key
  ON public.game_answers(game_session_id, question_id);

-- 3. Función idempotente: si ya respondió esa pregunta, retorna el id existente
...
BEGIN
  ...
  -- Si hubo conflicto (duplicado), new_id es NULL; devolver el id existente
